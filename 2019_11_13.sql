...
--fk 제약을 dept_test.deptno 컬럼을 참조하도록 생성하여 1번이외의 부서번호는 emp_test 테이블에 입력 될 수 없다.

--emp_test fk 테스트 insert
INSERT INTO emp_test VALUES (9999, 'borwn', 1);
...
DROP TABLE emp_test;
DROP TABLE dept_test;--이거를 emp_test보다 먼저 지우려고 하면 오류가 뜬다.
...
CREATE TABLE emp_test(--유니크와 프라이머리 키가 없다면서 에러남
    empno NUMBER(4),
    ename VARCHAR2(10),
...
--fk제약 default 옵션시에는 부모 테이블의 데이터를 삭제하기 전에 자식 테이블에서 참조하는 데이터가 없어야 정상적으로 삭제가 가능했음
--ON DELETE CASCADE의 경우 부모 테이블 삭제시 참조하는 자식 테이블의 데이터를 같이 삭제한다.
--1. 삭제 쿼리가 정상적으로 실행 되는지?
--2. 자식 테이블에 데이터가 삭제 되었는지?
DELETE dept_test--삭제하면 emp_test에 있는 내용도 사라진다. 만약 조건절을 걸지않고 테이블 통째로 삭제하면 emp_test도 통째로 날아간다.
WHERE deptno = 1;--삭제 쿼리가 정상적으로 실행 되는지?
SELECT *
FROM emp_test;

--fk제약3 ON DELETE SET NULL
DROP TABLE emp_test;
CREATE TABLE emp_test(
    empno NUMBER(4) PRIMARY KEY,
    ename VARCHAR2(10),
...
--fk제약 default 옵션시에는 부모 테이블의 데이터를 삭제하기 전에 자식 테이블에서 참조하는 데이터가 없어야 정상적으로 삭제가 가능했음
--ON DELETE SET NULL의 경우 부모 테이블 삭제시 참조하는 자식 테이블의 데이터의 참조 컬럼을 NULL로 수정한다.
--1. 삭제 쿼리가 정상적으로 실행 되는지?
--2. 자식 테이블에 데이터가 삭제 되었는지?
DELETE dept_test--삭제하면 emp_test에 있는 내용도 사라진다. 만약 조건절을 걸지않고 테이블 통째로 삭제하면 emp_test도 통째로 날아간다.
WHERE deptno = 1;--삭제 쿼리가 정상적으로 실행 되는지?
...
--SELECT 쿼리
--> CTAS('CREATE TABLE 테이블명 AS'의 약자)

DROP TABLE emp_test;
DROP TABLE dept_test;

--customer 테이블을 사용하여 COUSTOMER_TEST테이블로 생성
--customer 테이블의 데이터도 같이 복제
--제약 조건(PRIMARY KEY, NOT NULL)은 복제가 안된다.
...
SELECT sysdate dt --별칭을 주어야 생성된다.
FROM dual;
...
--데이터는 복제하지 않고 특정 테이블의 컬럼 형식만 가져올 수는 없을까?
...
--3. emp 테이블의 deptNO컬럼으로 dept 테이블의 deptno 컬럼을 참조하는 fk제약을 테이블 변경 
--ddl을 통해 생성
--emp --> dept (deptno)
SELECT *
FROM emp;
